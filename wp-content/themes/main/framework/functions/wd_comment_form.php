<?php
	if(!function_exists ('tvlgiao_wpdance_comment_form')){
		function tvlgiao_wpdance_comment_form( $args = array(), $post_id = null ) {
			...
			$defaut = array(
				'comment_author'		=>	esc_html__('Name*','wpdance'),
			...
			$fields =  array(
				'author' => '<div class="col"><span class="label">'.esc_html__('Your name','wpdance').'</span><p class="comment-form-author">' . '<input id="author" class="input-text" name="author" type="text" placeholder="' .$comment_author. '" data-default="'.$defaut['comment_author'].'" size="30"' . $aria_req . ' />' .'</p></div>',
				'email'  => '<div class="col"><span class="label">'.esc_html__('Your email','wpdance').'</span><p class="comment-form-email"><input id="email" class="input-text" name="email" type="text" placeholder="' . $comment_author_email . '" size="30"' . $aria_req . ' data-default="'.$defaut['comment_author_email'].'"/>'.'</p></div>',
			...
			$required_text = sprintf( ' ' . wp_kses(__('Required fields are marked %s','wpdance'), array()), '<span class="required">*</span>' );
			$defaults = array(
				'fields'               => apply_filters( 'comment_form_default_fields', $fields ),
				'comment_field'        => '<span class="label">'.esc_html__('Your message', 'wpdance').'</span><p class="comment-form-comment"><textarea id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea></p>',
				...
				'id_form'              => 'commentform',
				'id_submit'            => 'submit',
				'title_reply'          => esc_html__( 'Be first to comment','wpdance' ),
				'title_reply_to'       => esc_html__( 'Leave a Reply to %s','wpdance'),
				'cancel_reply_link'    => esc_html__( 'Cancel reply','wpdance' ),
				'label_submit'         => esc_html__( 'POST COMMENT','wpdance' ),
